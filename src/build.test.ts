...
describe("production input", () => {
  test("loads the current portfolio without historical brand or fleet projections", async () => {
    const root = await temporaryRoot();
    const portfolio = await Bun.file(
      new URL("../../project-governance/ecosystem/portfolio.v1.json", import.meta.url),
    ).json();
    await writeFixture(
      root,
      "node_modules/@libre-ai/governance/ecosystem/portfolio.v1.json",
  ...
  test("writes and reads back the exact bilingual local artifact", async () => {
    const expected = await buildProductionSite(process.cwd());
    const artifactRoot = join(await temporaryRoot(), "site");
    await writeStaticSite(artifactRoot, expected);
    expect((await listRelativeFiles(artifactRoot)).sort()).toEqual([
      "assets/styles.css",
      "assets/tokens.css",
      "index.en.html",
      "index.html",
    ...
    expect(expected.get("index.html")).toContain("Travailler et apprendre");
    expect(expected.has("comparaisons.html")).toBe(false);
    expect(expected.has("marque.html")).toBe(false);
  });
});
