...
interface StaticPortfolioSiteInput {
...
const productionSources = {
  portfolio: "node_modules/@libre-ai/governance/ecosystem/portfolio.v1.json",
  uiStyles: "node_modules/@libre-ai/ui/src/styles.css",
  uiTokens: "node_modules/@libre-ai/ui/src/tokens.css",
} as const;

export function buildStaticPortfolioSite(
  input: StaticPortfolioSiteInput,
): ReadonlyMap<string, string> {
  const portfolio = parsePortfolio(input.portfolio);
  const files = new Map([
    ["index.html", renderPortfolio(portfolio, "fr")],
    ["index.en.html", renderPortfolio(portfolio, "en")],
    ["assets/styles.css", `${input.uiStyles.trimEnd()}\n\n${renderSiteCss()}`],
    ["assets/tokens.css", input.uiTokens],
  ]);
  for (const [path, contents] of files) {
    if (findRemoteAssetReferences(contents).length > 0)
      throw new Error(`brand.output_remote_asset:${path}`);
    if (path.endsWith(".html") && findExecutableMarkup(contents).length > 0)
...
export async function loadProductionSiteInput(root: string): Promise<StaticPortfolioSiteInput> {
  const [portfolio, uiStyles, uiTokens] = await Promise.all([
    readFile(join(root, productionSources.portfolio), "utf8").then((value) => JSON.parse(value)),
    readFile(join(root, productionSources.uiStyles), "utf8"),
    readFile(join(root, productionSources.uiTokens), "utf8"),
  ]);
  return { portfolio, uiStyles, uiTokens };
}
...
export async function buildProductionSite(root: string): Promise<ReadonlyMap<string, string>> {
  return buildStaticPortfolioSite(await loadProductionSiteInput(root));
}
